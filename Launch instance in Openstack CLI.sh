...
sudo cat /etc/kolla/neutron-server/ml2_conf.ini | grep flat_network | awk '{print $3}' physnet1
...
openstack security group create security-group-allow-ssh-icmp --description 'Allow SSH and ICMP'
openstack security group rule create --protocol icmp security-group-allow-ssh-icmp
openstack security group rule create --protocol tcp --ingress --dst-port 22 security-group-allow-ssh-icmp
openstack security group rule create --protocol tcp --ingress --dst-port 3000 security-group-allow-ssh-icmp
openstack security group rule create --protocol tcp --ingress --dst-port 6443 security-group-allow-ssh-icmp
...
#create snapshot
 openstack server image create master-instance --name jammy-kubeadm-snapshot
